...
\begin{ptcbr}
    \begin{enumerate}[i)]
        ...
        To that effect, let $(x_n)_{n\in\bN}\subseteq F$ be a sequence in $F$ and observe that as $F$ is bounded, then there is a ball which completely covers $F$. Let us, for the sake of argument, consider a ball in $\infty$-norm. Observe that if we bisect the enclosing ball we will be left with two halves each with volume $M^3/2$. Since our sequence has an infinite number of points, we may apply the pigeonhole principle to deduce that one of the halves has infinitely many points. 
        \item \red{FINISH}
