...
\begin{enumerate}
    ...
    To see linear independence it suffices to see orthogonality. Between different index sizes it is clear via hypothesis. So let $I,J\in\binom{\bonj{n}}{k}$ and consider $\braket{e_I}{e_J}$. The obtained quantity is the determinant of the Gram matrix formed by the bases $\set{e_i}_{i\in I}$ and $\set{e_j}_{j\in J}$.\red{What if we get a permutation matrix? Is it even possible? like e1e3e4 and e4e1e3}.
...
So to our effect, assume $\om$ is exact. Then there is an $f\: M\to\bR$ such that $\dd f=\om$. As $M$ is compact, then there exists a point $p\in M$ at which $f$ attains a minimum (otherwise, we'll just have to switch signs). \red{FINISH}
\end{ptcbr}
\end{document} 