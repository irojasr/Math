...
\begin{Ej}[Stein \& Shakarchi 2.15]
    ...
    $$|f(z)|=1\word{when}|z|=1,$$
    then $f$ is constant. \hint{Extend $f$ to $\bC$ by $f(z)=\frac{1}{\ov{f(1/\ov z)}}$ when $|z|>1$. Argue as in the Schwarz reflection principle.}\hint{You will need to use the fact that, away from $0$, $z\mapsto\frac1z$ is continuous; so $z$ and $z_0$ are close if and only if $\frac1z$ and $\frac{1}{z_0}$ are close.}
\end{Ej}
...
\begin{Ej}
    ...
    \begin{enumerate}[i)]
        \itemsep=-0.4em
        \item Find the residue $\res(f,2)$.\hint{Expand $f(z)$ in terms of powers of $(z-2)$.}
...
\section{Day 16| 20230301}

Recall we have talked about removable singularities. 
%%%%%%%%%%%% Contents end %%%%%%%%%%%%%%%%
\ifx\nextra\undefined
\printindex
\else\fi
