...
\begin{Def}
    We say $H^0(D)$ is \term{basepoint-free} if for every $p\in C$, there is $\vf\in  H^0(D)$ such that $\vf(p)\neq 0$.
\end{Def}

This condition guarantees that the image of the map $\vf_D\: C\to\bP^{h^0(D)-1}$ has degree $\deg(D)$. Because we check the degree by intersecting with a hyperplane. We might as well check with $x_0=0$. But the pullback is the \red{section? Check video}.

\begin{Ex}
    $H^0(D-p)$ is an example of a non-basepoint-free. \red{what?}
\end{Ex}
...
Recall that from the proof of Riemann-Roch, subtracting a point should \red{something.}
...
\begin{ptcbp}
    ...
    \end{itemize}
    If $C$ is hyperelliptic, we have a function of degree $2$ so that we can pick $p,q$ to be two points mapping to the same point so that $h^0(P+Q)\geq 2$\red{Finish}.\par
    On the other hand 
    \begin{align*}
...
$H^0(D)$ is still basepoint-free so in a sense \red{what?}
$$\vf_{K_C}\: C\to\bP^{g-1}$$
...
\begin{Def}
    We say that a divisor is \term{very ample} whenever the associated $\vf_D$ is an embedding. It is \term{ample} when it has a very ample positive multiple.
\end{Def}
...
\begin{Def}
    The space $\operatorname{Hurw}^d$ is the space of \term{Hurwitz covers} of degree $d$ of $\bP^1$,
...
\begin{Def}
    ...
    \begin{center}
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAGEB9ARhAF9S6TLnyEU3clVqMWbLgCZ+gkBmx4CRMtyn1mrRCAA6hgEYAFAHq8BQtaKITt1XbIPHzV-lJhQA5vCJQADMAJwgAWyR5ahwIJABmZxl9I0MaIJBqBjoTGAYzYXUxEBCsXwALHCVgsMjEMhBYqKS9NiCeTJBs3PzC+wNSiqqbEFCIpAlGuMRE6VaDdsURsbqGpsRJlxTjM3KsLz4gA
\begin{tikzcd}
    C_1 \arrow[d, "f_1"'] \arrow[r, "\Phi"] & C_2 \arrow[d, "f_2"] \\
    \bP^1 \arrow[r, "\vf"']                 & \bP^1               
    \end{tikzcd}
    \end{center}
\end{Def}
