...
\begin{Def}
    A \term{Hadamard matrix} is a matrix with entries in $\set{1,-1}$ such that $HH^\sT=nI$. 
\end{Def}
...
\begin{Def}
    A \term{duality of designs} is a pair of morhpisms $(\sg,\tau)$ such that 
    $$\tau(B)\in \sg(X).$$
    If $\sg\tau=\id_B$ and $\tau\sg=\id_X$ we say that the duality is a \term{polarity}.
\end{Def}
...
\begin{Def}
A \term{strongly regular graph} with parameters $(n,k,\la,\mu)$ is a graph with $n$ vertices, not complete nor empty such that for every pair of vertices $x,y$, the number of \textbf{common neighbors} is 
$$
\left\lbrace
\begin{aligned}
    &k\word{if}x=y\\
...
\begin{Def}
    A \term{matroid} is a pair $(E,\cI)$ where $E$ is a finite set and $\cI\subseteq\cP(E)$, called he set of independent sets, satisfying the following properties:
    \begin{itemize}
        \itemsep=-0.4em
        \item $\emptyset\in\cI$.
...
In this case we have a \term{dependence relation} among $v_1,\dots,v_n$ which means that 
...
\begin{Def}
    A \term{basis of a matroid} is a maximal independent set under the inclusion relation. 
\end{Def}
...
\begin{Def}
    A \term{matroid} is a pair $(E,\cB)$, where $E$ is finite and $\cB\subseteq\cP(E)$ such that 
    \begin{itemize}
        \item $\cB$ is nonempty.
...
\begin{Def}
    Suppose $M=(E,\cI)$ is a matroid and $X\subseteq E$. The \term{restriction} of $M$ from $E$ to $X$ is 
    $$M\mid_{X}=(X,\cI\mid_X)\word{where}\cI\mid_X=\set{I\in\cI\:\ I\subseteq X}.$$
    The \term{rank} of a matroid is the size of any basis. For a subset $X$, the rank is $\rk(M\mid_X)$, the size of the largest independent set contained in $X$.
\end{Def}

In graphs, rank is largest spanning tree. With this we can rank-define a matroid.

\begin{Def}
    A \term{matroid} is a finite set $E$ with a rank function $\rk\: \cP(E)\to\bN$ which satisfies 
    \begin{enumerate}[i)]
        \itemsep=-0.4em
...
\begin{Def}
    If $M=(E,\rk)$ is a matroid, then for $X\subseteq E$ we define the \term{closure} as 
...
\begin{Def}
    A \term{matroid} is a pair $(E,\cl)$ if $\cl\:\cP(E)\to\cP(E)$
    \begin{enumerate}[i)]
        \itemsep=-0.4em
...
\begin{Def}
    A \term{flat} of a matroid $M$ is a set $X\subseteq E$ such that $X=\cl(X)$. A \term{hyperplane} is a flat of rank $\rank(M)-1$.
\end{Def}

\begin{Def}
    The \term{lattice of flats} of a matroid $M$ is the poset of all flats ordered by inclusion. 
\end{Def}

This is a very important construction, besides bases, this is the other way that we think about matroids.

\begin{Ex}[Ex. 1.7.7 \cite{OxleyMatroids} (pg. 64)]
    Consider the graphical matroid $M$
\begin{center}
...
%Shape: Square [id:dp5214198384559152] 
\draw   (100,100) -- (165.93,165.93) -- (100,231.86) -- (34.07,165.93) -- cycle ;
%Curve Lines [id:da02050181332796097] 
\draw    (100,100) .. controls (69,161.5) and (69,170.5) .. (100,231.86) ;
%Curve Lines [id:da5899162790450347] 
\draw    (100,100) .. controls (135,159.5) and (135,173.5) .. (100,231.86) ;
%Shape: Circle [id:dp06599405508210099] 
\draw   (165.93,165.93) .. controls (165.93,155.97) and (174,147.89) .. (183.96,147.89) .. controls (193.92,147.89) and (202,155.97) .. (202,165.93) .. controls (202,175.89) and (193.92,183.96) .. (183.96,183.96) .. controls (174,183.96) and (165.93,175.89) .. (165.93,165.93) -- cycle ;

% Text Node
\draw (65.44,129.6) node [anchor=south east] [inner sep=0.75pt]    {$1$};
% Text Node
\draw (62.44,216.6) node [anchor=south east] [inner sep=0.75pt]    {$2$};
% Text Node
\draw (148.44,216.6) node [anchor=south east] [inner sep=0.75pt]    {$3$};
% Text Node
\draw (146.44,126.6) node [anchor=south east] [inner sep=0.75pt]    {$4$};
% Text Node
\draw (74.44,170.6) node [anchor=south east] [inner sep=0.75pt]    {$5$};
% Text Node
\draw (143.44,170.6) node [anchor=south east] [inner sep=0.75pt]    {$6$};
% Text Node
\draw (221.44,171.6) node [anchor=south east] [inner sep=0.75pt]    {$7$};


\end{tikzpicture}

\end{center}
    We have a loop and a 2-cycle. The lattice is denoted $\cL(M)$, the closure of the empty set in this matroid is $\set{7}$, the full rank of the matroid is $3$ so we may write the poset as follows:
    \begin{center}
        GRAPh
    \end{center}
    If we were going to build a flat of rank $3$, we could only obtain the whole set. There's gonna be some rank $1$ and $2$.\par 
    This is a lattice! Every pair of elements has a \emph{meet} and a \emph{join}.
\end{Ex}
...
\begin{Def}
    A \term{loop} is a circuit of size $1$, where a circuit is minimal dependent set.
\end{Def}
...
\begin{Def}
    The \term{parallel class} of $x\in E$ is $\set{y\: \set{x,y}\in\cC}$.
\end{Def}

In a representable matroid, the parallel class are the linearly dependent elements.

\begin{Def}
    A matroid $M$ is \term{simple} if it has no loops or $2$-circuits.\par 
    The \term{simplification} of $M$, $M_\sim$ is formed by
    \begin{itemize}
...
\begin{Th}
    A finite poset is the lattice of flats of a matroid if and only if it is a \term{geometric lattice}.
\end{Th}

Recall atoms on posets are the elements just above $0$, the rank 1 elements of a poset. A lattice is \term{atomic} if every element is made up of atoms. In other words every element is a join of atoms.

\begin{Def}
    A \term{geometric lattice} is a graded, atomic and upper semi-modular lattice.
\end{Def}
...
In terms of matroids, we have $\rk$ as a grading and this is already upper semi-modular. 
 ...
 \begin{Def}
    A \term{coatomic lattice} is a lattice where every element is the meet of coatoms (elements below $1$).
 \end{Def}
 ...
 Recall that for weighted graphs we have \term{Kruskal's algorithm}. Consider the graph
\begin{figure}
    fig1
\end{figure}
...
\begin{Def}
    For a graph $G$, the \term{contraction} of $G$ at $e$ is $G/e$ is 
    \begin{figure}
        figure
    \end{figure}
    The \term{deletion} is $G\less e$. A graph that can be formed from deletion and contraction from $G$ is a \term{minor} of $G$.
\end{Def}

In general there exist $k$-minors which allow us to only do $k$ steps. \term{Ulam's conjecture} says that every graph can be uniquely determined by its $1$-vertex deletions (DIFFERENT THATN THIS OPERATION). This is a reconstructibility problem.\par 
There's other question on reconstructibility for families of graphs. 

\begin{Def}
    A \term{planar graph} is a graph that can be embedded into the plane and has no crossing edges.
\end{Def}
...
%Contraction is defined using duality $M/e=(M^\ast\e)^\ast$.

\begin{Def}
    The \term{dual matroid} of $M=(E,\cB)$ is $M^\ast=(E,\set{E-B\: B\in\cB})$.
\end{Def}
...
\begin{Def}
A \term{proper coloring} of a graph $G$ is a coloring such that no two adjacent vertices is the same color. 
\end{Def}
...
\begin{Def}
    A sequence $(a_n)$ is \term{log-concave} if $(\log(a_n))$ is a concave sequence. Or Equivalently $a_n^2\geq a_{n-1}a_{n+1}$.
\end{Def}
...
\begin{Def}
    The \term{Grassmannian} is the set of $k$-dimensional subspaces of $\bF^n$. We denote it $Gr_\bF(k,n)$.
\end{Def}
...
where the disjoint union is taken over partitions formed in the previous way \red{ASK HOW lambda subset box???} and $\Om_\la^0$ is the set of row-reduced $k\x n$ matrices with pivots in columns $\la_k+1$, $\la_{k-1}+2$ all through $\la_1+k$.
\end{Prop}
...
\begin{Def}
    We define the \term{projective linear group} as 
...
\begin{Def}
A \term{flag} is a chain of subspaces 
...
\begin{Def}
    A \term{Flag variety} is the set of flags in $\bF^n$. This is denoted:
...
\begin{Ex}
    ...
    To simplify our matrix we can rescale our matrix and combine rows with \emph{lower} rows. The obtained matrix is said to be in \term{normal form}.
\end{Ex}
...
\begin{Ex}
    ...
    $$\begin{pmatrix}
        ...
        0&0&0&0&0&1&c
    \end{pmatrix}$$
    which represents \red{something}, this is associated to the tableau 
    $$\young(xyza,~~wk,~~~c)$$
...
\begin{Def}
    An \term{algebraic variety} is the set of points defined by polynomial equations. When it's projective, the polynomials must be homogenous.
\end{Def}
...
\begin{Def}
    Two varieties $V,W$ are \term{rationally equivalent} if there exist homogenous (in $\un x)$ polynomials $f_i(\un x,t)$ such that 
...
\begin{Def}
    The \term{Chow ring} of a variety $X$ is the set of subvarieties $V\subseteq X$ up to rational equivalence.
\end{Def}
