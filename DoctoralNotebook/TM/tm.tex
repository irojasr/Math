...
\begin{Ex} 
    ...
    Here the $\psi_i$ inside the pullback is supported on $\ov M_{g,n}$ whereas the other one in $\ov{M}_{g,n+1}$. Graphically this would be represented by two stable graphs, one decorated and the other one not.\red{Fig}
\end{Ex}
...
\begin{Ex}%https://mathoverflow.net/questions/100213/pull-back-of-hodge-bundle-via-glueing-map
    ...
    We can express this graphically for the first lambda-class.\red{fig}
...
\begin{Ex}
    Let's begin with a sanity check. Consider the product in $\ov M_{0,4}$ of the fundamental class $1$ with $\dl_{0,{ac}}$. The product is represented graphically as\red{fig}\par
...
\begin{Ex}\red{fig}
    Consider now the product $\dl_{0,{ac}}^2$ in $\ov{M}_{0,4}$. As this boundary divisor has codimension $1$, its square should have codimension $2$. As $\ov{M}_{0,4}$ has only one dimension, we expect the product to be empty. The representation of this product graphically can be seen in \red{fig}.\par
    ...
    \begin{enumerate}
        ...
        \item The edge in the result is bicolored to indicate that it is identified with the edges in the factors. Here we label the half-edges as $\8$ and $\ast$ so that we must multiply a psi-class $(-\psi_\8-\psi_\ast)$. This is the only difference with the previous calculation.\red{fig}
    ...
    and when pulling back the psi-classes via the gluing map to the corresponding $M_{0,3}$'s we get the same psi-classes. These are $0$ in $M_{0,3}$ so that the result is also 0.\red{fig}
\end{Ex}
...
\begin{Ex}
    ...
    We may represent this product as the following graphs.\red{Add drawings}\par
    The expected codimension of the result is $3$ as every factor contributes $1$ codimension.\par

    We begin by multiplying a copy of $\dl_{0,\set{ab}}$ with $\dl_{0,\set{a,b,c}}$:\red{add drawing with edges colored}.\par
\red{FINISH WRITING}
...
\begin{Ex}[\cite{ZvonkineIntro}, Exercise 2.4]\label{ex-unexpectedly-zero-square}
   Recall the irreducible divisor of stable curves with one non-separating node $\dl_{\text{irr}}\in\ov{M}_{1,n}$. We will show that $\dl_{\text{irr}}^2=0$.\red{DO}
