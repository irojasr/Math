...
\begin{itemize}
    \item Finish writing examples on intersection products. Add examples on psi and lambda classes.
    ...
    \item Apply A.B. to proj plane and find the intersection of two lines.
...
\begin{Def}
    A \term{stratum} in $M_{g,n}$ is the closure of the set of all curves with the same topological type. Strata are classified by dual trees. 
\end{Def}

\begin{Ex}
    In $\ov M_{0,6}$, the fundamental class $1\in A^0(M_{0,6})$ is the class of all curves homeomorphic to a 6-pointed $\bP^1$. It can be represented by the dual graph:
    \begin{figure}[h!]
        \centering
...
\begin{enumerate}
    ...
    \item Consider all graphs who minor\footnote{The current lingo in geometry is specialize.} into the factors' dual graphs at the same time.
    \item Color the edges of the resulting graphs such that when collapsing edges, the original colorings are respected (\red{clarify respected}.) No edge of a resulting graph may be left \textbf{uncolored}.
    \item If an edge is bicolored multiply the corresponding $\psi$-classes. 
    \item Count number of automorphisms of the graph and divide the graph by that number. 
    \item Count the number of possible colorings which respect the conditions and multiply the graph by that number.
\end{enumerate}
...
\begin{Ex}
    Let's begin with a sanity check. Consider the product in $\ov M_{0,4}$ of the fundamental class with $D(ac\mid bd)$.\red{Add drawing}.\par
    The only graph which minors into both is exactly the one corresponding to $D(ac\mid bd)$ and it can only be colored in one way, and has no non-trivial automorphisms. Therefore 
...
\begin{Ex}
    ...
    To see this via the algorithm, we consider all graphs which minor into our tree. There's no graphs with two edges as that violates the stability condition (all vertices should have at least 3 special points), and so the only possible graph is a copy of $D(ac\mid bd)$ with its only edge bicolored.\par
    Label the marks $\ast$ and $\.$ on the edge of $D(ac\mid bd)$ so that we may see it as the image of the gluing morphism 
    $$gl\: M_{0,\set{a,c,\ast}}\x M_{0,\set{b,d,\.}}\to M_{0,4}.$$
    Then the $\psi$-classes we muliply are $(-\psi_{\.}-\psi_{\ast})$ so that the result is $D(ac\mid bd)\.(-\psi_{\.}-\psi_{\ast})$.
    \red{Add drawings}
\end{Ex}
...
\begin{Ex}
    Consider the intersection product 
    $$D(ab\mid cdef)^2 D(abc\mid def)$$
    which lives in $\ov{M}_{0,6}$. The expected codimension of the result is $3$ as every factor contributes $1$ codimension.\par
    We first multiply a copy of $D(ab\mid cdef)$ with $D(abc\mid def)$:\red{add drawing}.\par
    Graphs which have these two as minors are scarce. In fact, ¡there's only one: $D(ab\mid c\mid def)$!\par 
    Coloring the edges of our original factors \blu{blue} and \green{green} we get only one copy of our resulting graph with the coloring determined.\red{ADD DRAWING}\par
    ...
    We could expect the resulting graphs to have three edges as that would have codimension $3$. But take for example $D(ab\mid c\mid d\mid ef)$. Such graph has three edges but \textbf{they can't be colored}. When collapsing edges, the colors disappear so that we would have to leave the edge between $d$ and $ef$ uncolored to respect the coloring.\red{Add drawing}\par
    This cannot happen. So $D(ab\mid c\mid def)$ is the only possible factor. The edge between $ab$ and $c$ will be bicolored in a way that we have to multiply $\psi$-classes. Labeling the vertices attached to the edge as $\.$ and $\star$ we get 
...
\begin{Ex}[$(4,0)$ instead of $(0,4)$]
    Consider the curves represented by the dual graphs:\red{add drawing}\par
    Finding the graphs whose minors are these ones we get three possibilities:
    \begin{enumerate}
        \item The first graph itself. \red{FINISH}
    \end{enumerate}
\end{Ex}
