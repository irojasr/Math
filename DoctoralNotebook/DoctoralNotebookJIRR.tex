...
\begin{ptcbr}
    ...
    $$\div(s_0)=\sum_{i=1}^{m}[\al_i]-\sum_{j=1}^{n}[\bt_j]+(n-m+d)[\infty]\word{and}\div(s_1)=\sum_{i=1}^{r}[\ga_i]-\sum_{j=1}^{s}[\dl_j]+(s-r+d)[\infty].$$
    Their product is precisely 
    $$s_0(x)s_1(x)=\frac{\prod_{i=1}^{m+r}(x-\tilde\al_i)}{\prod_{j=1}^{n+s}(x-\tilde\bt_j)},$$
    where $\tilde{\al}_i$ corresponds to $\al_i$ when $i$ is between 1 and $m$
\end{ptcbr}
