...
In the pseudostable case, we can calculate the same GW invariant, just that now the calculation is a bit different because of cusps. Let us compute the genus 3, degree $d$ GW invariant:
...
\subsection{The $(3,0)$ contribution}
...
First we just expand the pseudostable Mumford relation to obtain
\begin{align*}
...
\begin{Th}[\cite{GetzlerAndPandharipande}, T.3]
...
\subsection{The $(2,1)$ contribution}
...
\subsection{The $(1,2)$ contribution}
...
\subsection{The $(0,3)$ contribution}
...
$$
which gives
$$\frac{e(\DefB(f))}{\Ob(f)}=\frac{e(H^0(\cO(2d\widetilde{H}-t)))}{e(H^1(\cO(-t)))}=\frac{\left(t/d\right)^{2d}d!^2(-1)^d}{(-1)^3\La_3^{\ps}(t)}$$
\end{itemize}
