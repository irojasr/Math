...
The fixed locus $F_{(i,j)}$ denotes the nodal curve consisting of three components: one of genus $i$ mapping to zero, the smooth component $\bP^1$ and the genus $j$ component mapping to infinity; this, along the information of the map. Let us analyze the contribution of $F_{(3,0)}$.\par
...
In the previous observation we have used the fact that $\ov{\cM}^{\ps}_{g,n}$ has the same dimension as its stable counterpart. This is mentioned in \cite{cavalieri2022pseudostablehodgeintegrals}, pp.5. We won't limit ourselves to only using this fact, we need two more results to further the calculation.

\begin{Th}[\cite{cavalieri2022pseudostablehodgeintegrals}, T.2.4]
...
=&2\glu_\ast^2\bigl(p_0^\ast(\la_1)p_1^\ast(\la_1)(\psi_{\star1}-\psi_{\81})(\psi_{\82}^2-\psi_{\star2}^2)\bigr)
\end{align*}
\end{ptcb}
