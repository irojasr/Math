...
\subsection{Rigorous revision: status, conventions, and conditional result}

This subsection replaces the exploratory calculation above.  The earlier text is
retained in the source inside an \verb|\iffalse| block so that the provenance of
the formulas remains visible.  Line numbers in the audit below refer to the
pre-revision file.

\subsubsection{What is and is not established}

The stable-map calculation of Faber--Pandharipande is a theorem on
\(\ov{M}_{g,0}(\bP^1,d)\): it uses its standard equivariant perfect obstruction
theory and virtual localization.  The available pseudo-stable sources establish
facts about \(\ov{M}^{\ps}_{g,n}\), its Hodge classes, and the contraction of
elliptic tails.  They do not, in the files available here, construct a proper
Deligne--Mumford stack \(\ov{\cM}^{\ps}_{g,0}(\bP^1,d)\) with an equivariant
perfect obstruction theory and prove a virtual localization theorem for it.
The file \texttt{DustyNotes .pdf} proposes the required graph rules and tests them in
examples; they do not supply that missing construction.  Accordingly, the
result below is conditional.

Fix the following conventions.  The tangent weights of \(\bP^1\) at
\(0\) and \(\infty\) are respectively
\[
  w_0=t,\qquad w_\infty=-t.
\]
The two copies of \(\cO_{\bP^1}(-1)\) have fiber weights
\([0,t]\) and \([-t,0]\).  Thus, if \(z=x^d\) on the degree-\(d\)
edge, the tangent weights on the source at its two ends are \(t/d\)
and \(-t/d\).  We use the homogeneous Hodge polynomial
\[
  \La_g^{\ps}(t):=\sum_{i=0}^g t^{g-i}\la_i^{\ps}.
\]
Here \(\bE^{\ps}:=\pi_*\omega_\pi\) on the moduli stack of
pseudo-stable curves and \(\la_i^{\ps}:=c_i(\bE^{\ps})\).  This convention
must also be the convention used by any future obstruction theory on the
pseudo-stable map space.
In particular,
\[
 e(\bE^\vee\otimes\bC_t)=(-1)^g\La_g^{\ps}(-t),\qquad
 e(\bE^\vee\otimes\bC_{-t})=(-1)^g\La_g^{\ps}(t).
\]

We isolate the assumptions that are not supplied by the available sources.

\begin{enumerate}
  \item[(A1)] There is a proper pseudo-stable map stack with a
  \(\bC^*\)-equivariant perfect obstruction theory whose restriction to the
  nodal one-edge loci is represented by the usual complex
  \(R\pi_*f^*T\bP^1\), together with the usual deformation theory of the
  source, and virtual localization holds for this theory.

  \item[(A2)] For the chosen linearizations, the only nonzero graph
  contributions are the proposed one-edge graphs.  A genus-one decoration at
  an end is represented by the rational-cusp vertex of
  \texttt{DustyNotes .pdf}, not
  by a contracted elliptic tail.

  \item[(A3)] For \(g\geq2\) and \(0\leq k\leq g-1\), the following
  integrated comparison identity holds:
  \begin{align*}
   &\int_{\ov M^{\ps}_{g,1}}
     \la_g^{\ps}\,\glu_*^k
     \left(\prod_{j=1}^k(\psi_{\star j}-\psi_{\8 j})\right)
     \psi_P^{2g-2k-2}\\
   &\hspace{1cm}=
     \left(\int_{\ov M_{g-k,1+k}}
       \la_{g-k}\psi_P^{2g-2k-2}\prod_{j=1}^k\psi_{\star j}\right)
     \left(\int_{\ov M_{1,1}}\la_1\right)^k.
  \end{align*}
  The formulas cited in the old draft for linear pseudo-stable Hodge
  integrals and for the quadratic pseudo-stable Mumford class do not, by
  themselves, prove this product identity.  It requires a projection-formula
  and self-intersection calculation for the elliptic-tail strata.
\end{enumerate}

\begin{Th}[Conditional one-edge factorization]\label{th:conditional-ps-factorization}
Assume \emph{(A1)--(A3)} and let \(g_1,g_2\geq2\).  Let
\(F_{g_1,g_2}\) be the one-edge fixed locus consisting of a degree-\(d\)
rational component, a contracted genus-\(g_1\) component over \(0\), and a
contracted genus-\(g_2\) component over \(\infty\).  Then
\[
 F_{g_1,g_2}\cong
 \left[(\ov M^{\ps}_{g_1,1}\times\ov M^{\ps}_{g_2,1})/\mu_d\right].
\]
After removing the residual factor \(1/d\) coming from \(\mu_d\), its
localization contribution is
\[
 I_{(g_1,g_2)}=d^2 I_{g_1}I_{g_2},
\]
where, for \(g\geq2\),
\begin{equation}\label{eq:correct-Ig-integral}
 I_g:=\int_{\ov M^{\ps}_{g,1}}
 \frac{\la_g^{\ps}\La_g^{\ps}(t)(-1)^g\La_g^{\ps}(-t)}
 {t^2(1-d\psi_P/t)}.
\end{equation}
The full stack contribution of this locus is therefore
\(dI_{g_1}I_{g_2}\).
\end{Th}

\begin{proof}
The deformation--obstruction sequence, restricted to moving parts, is
\[
 0\to\operatorname{Aut}(C)\to H^0(C,f^*T\bP^1)
 \to T^1_{[f]}\to\operatorname{Def}(C)
 \to H^1(C,f^*T\bP^1)\to T^2_{[f]}\to0.
\]
Here \(\operatorname{Def}(f):=H^0(C,f^*T\bP^1)\) and
\(\operatorname{Ob}(f):=H^1(C,f^*T\bP^1)\).
Consequently the convention used in the old draft is
\[
 e(N^{\vir})=
 \frac{e(\operatorname{Def}(f))^{\mathrm{mov}}
 e(\operatorname{Def}(C))^{\mathrm{mov}}}
 {e(\operatorname{Aut}(C))^{\mathrm{mov}}
 e(\operatorname{Ob}(f))^{\mathrm{mov}}}.
\]
The degree-\(d\) component has a zero-weight infinitesimal scaling; it is
fixed and hence does not enter this Euler class.  Its residual finite
automorphism group is \(\mu_d\), which produces the separate stack factor
\(1/d\).

The moving map deformations on the edge have weights
\(t-kt/d\), for \(0\leq k\leq2d\), with the zero weight \(k=d\)
removed.  Hence
\[
 e\bigl(H^0(\bP^1,f^*T\bP^1)^{\mathrm{mov}}\bigr)
 =(-1)^d\left(\frac td\right)^{2d}(d!)^2.
\]
The two obstruction-bundle insertions restricted to the edge contribute
\[
 (-1)^{d-1}\left(\frac td\right)^{2d-2}((d-1)!)^2.
\]
Their quotient is exactly
\[
 \frac{(-1)^{d-1}(t/d)^{2d-2}((d-1)!)^2}
 {(-1)^d(t/d)^{2d}(d!)^2}=-\frac1{t^2}.
\]

Let \(P_i\) be the attaching marking on the contracted component.  The two
node-smoothing weights are
\[
 \frac td-\psi_{P_1}
 =\frac td\left(1-\frac{d\psi_{P_1}}t\right),\qquad
 -\frac td-\psi_{P_2}
 =-\frac td\left(1+\frac{d\psi_{P_2}}t\right).
\]
After inversion their product contributes
\[
 -\frac{d^2}{t^2}
 \frac1{(1-d\psi_{P_1}/t)(1+d\psi_{P_2}/t)}.
\]
Combining this with the edge quotient gives \(d^2/t^4\).  The two
local-curve obstruction insertions and the two factors from
\(H^1(C,f^*T\bP^1)\) supply the Hodge numerators, so the remaining class is
\begin{align*}
 d^2
 &\left(
 \frac{\la_{g_1}^{\ps}\La_{g_1}^{\ps}(t)(-1)^{g_1}
 \La_{g_1}^{\ps}(-t)}{t^2(1-d\psi_{P_1}/t)}
 \right)\\
 &\qquad\cdot
 \left(
 \frac{\la_{g_2}^{\ps}\La_{g_2}^{\ps}(-t)(-1)^{g_2}
 \La_{g_2}^{\ps}(t)}{t^2(1+d\psi_{P_2}/t)}
 \right).
\end{align*}
This proves the claimed factorization before the \(\mu_d\)-quotient.

It remains to compare the two tail integrals.  The homogeneous quadratic
relation has terms of codimension \(2k\) multiplied by \(t^{2g-2k}\).
Since \(\dim\ov M^{\ps}_{g,1}=3g-2\), the only term from the geometric
series that can pair with it and \(\la_g^{\ps}\) is
\(\psi_P^{2g-2k-2}\).  This exponent is even.  Replacing \(t\) by
\(-t\) and \(d\psi_P/t\) by \(-d\psi_P/t\) therefore does not change
the integral.  Moreover, the factor \(t^{2g-2k}\), the prefactor
\(t^{-2}\), and the chosen geometric-series term cancel to total
equivariant degree zero.  This is the precise reason that the two tails agree; it is
not a general permission to discard the sign of \(\psi\).
\end{proof}

\subsubsection{Evaluation of the tail and the exceptional genera}

Set
\[
 b_0:=1,\qquad
 b_g:=\int_{\ov M_{g,1}}\psi_1^{2g-2}\la_g
 =\frac{2^{2g-1}-1}{2^{2g-1}}\frac{|B_{2g}|}{(2g)!}
 \qquad(g\geq1).
\]
Under assumption \emph{(A3)}, the \(\lambda_g\)-formula gives, for
\(g\geq2\),
\begin{equation}\label{eq:correct-Ig-sum}
 \boxed{
 I_g=\sum_{k=0}^{g-1}
 \frac{d^{2g-2k-2}}{k!}
 \frac{(2g-k-2)!}{(2g-2k-2)!}\,
...
\[
 \int_{\ov M_{g-k,1+k}}
 \la_{g-k}\psi_P^{2g-2k-2}\prod_{j=1}^k\psi_{\star j}
 =\frac{(2g-k-2)!}{(2g-2k-2)!}\,b_{g-k},
\]
and \(\int_{\ov M_{1,1}}\la_1=1/24\).  The upper bound is
\(g-1\), because the exponent of \(\psi_P\) must be nonnegative.

There is no geometric integral \eqref{eq:correct-Ig-integral} for an
unstable genus-zero vertex, and a genus-one end of a pseudo-stable map is a
rational cusp rather than a contracted elliptic tail.  Thus the values
\[
 I_0:=\frac1{d^2},\qquad
 I_1:=\frac{d^2-1}{24d^2}
\]
are conventions encoding, respectively, the unstable vertex and the
genus-one cusp rule proposed in \texttt{DustyNotes .pdf}.  They are not obtained
from negative factorials and they remain conditional on \emph{(A1)--(A2)}.
With these conventions the uniform bookkeeping identity
\[
 I_{(g_1,g_2)}=d^2I_{g_1}I_{g_2}
\]
also covers ends decorated by \(0\) or \(1\).

\subsubsection{Generating functions and coefficient conventions}

As a formal power series, let
\[
 B(u):=\sum_{g\geq0}b_gu^{2g}=\frac{u/2}{\sin(u/2)},
 \qquad
 \cI(u):=\sum_{g\geq0}I_gu^{2g}.
\]
Then
\begin{equation}\label{eq:correct-I-generating}
 \boxed{
 \cI(u)=\frac1{d^2}\left(1-\frac{u^2}{24}\right)
 B\left(\frac{du}{1-u^2/24}\right)
 =\frac{u/(2d)}{\sin\!\left(\dfrac{du}{2(1-u^2/24)}\right)}.}
\end{equation}
This statement is purely formal; no appeal to analytic Fubini is needed.
Equivalently,
\[
 I_g=\sum_{n=0}^g d^{2n-2}b_n
 [x^{g-n}](1-x/24)^{1-2n}.
\]
For \(n=0\), the factor is \(1-x/24\); this gives \(I_0\) and the
correction \(-1/(24d^2)\) in \(I_1\), without assigning values to
negative factorials.  For \(g\geq2\), coefficient extraction is exactly
\eqref{eq:correct-Ig-sum}.  Conversely, for \(g\geq2\),
\[
 b_g=d^{2-2g}\left(
 I_g-\sum_{k=1}^{g-1}
 \frac{d^{2g-2k-2}}{k!}
 \frac{(2g-k-2)!}{(2g-2k-2)!}\frac{b_{g-k}}{24^k}
 \right).
\]

If the proposed graph sum in \emph{(A1)--(A2)} is valid for every genus,
then the full invariant has the conditional generating series
\begin{equation}\label{eq:correct-Cps-generating}
 \sum_{g\geq0}C^{\ps}(g,d)u^{2g}
 =d\cI(u)^2
 =\frac{u^2/(4d)}
 {\sin^2\!\left(\dfrac{du}{2(1-u^2/24)}\right)}.
\end{equation}
By comparison, the established stable-curve calculation of
Faber--Pandharipande gives
\[
 C(g,d)=d^{2g-3}\sum_{g_1+g_2=g}b_{g_1}b_{g_2},\qquad
 \sum_{g\geq0}C(g,d)u^{2g}=d^{-3}B(du)^2.
\]
What transfers to the pseudo-stable calculation is the edge weight algebra,
the two smoothing factors on a nodal one-edge locus, the residual
\(\mu_d\)-factor, and the stable \(\lambda_g\)-integral used after a valid
comparison to \(\ov M_{g,n}\).  What does not transfer without proof is the
map stack, its virtual class, localization, the completeness of the fixed
graph list in the presence of cusps, the cusp-vertex deformation factors,
and the integrated identity \emph{(A3)}.

\subsubsection{Audit of the original argument}

The classifications below refer to the original line numbers.

\begin{enumerate}
 \item \textbf{My()Theorem.tex, lines 29--31; DoctoralNotebookJIRR.tex,
 lines 1554--1556 --- conjectural.}  The formula treats a pseudo-stable
 virtual class and localization as available.  Neither
 \texttt{DustyNotes .pdf} nor \texttt{SchubertNewComp.tex} proves the
 existence of the required map-space obstruction theory.  Replacement:
 assumptions \emph{(A1)--(A2)} above.

 \item \textbf{My()Theorem.tex, lines 33--37 --- incorrect as a complete
 fixed-locus statement.}  The listed nodal graphs omit rational cuspidal
 source components, which \texttt{DustyNotes .pdf}, pp.~1--5, explicitly
 identifies as new fixed loci.  The list is usable only after the
 linearization vanishing and the cusp rules are proved.

 \item \textbf{My()Theorem.tex, lines 82--124 --- ambiguous.}  Two
 incompatible definitions of \(\La_g^{\ps}\) occur.  The homogeneous
 definition fixed above is the one required by the Euler classes.

 \item \textbf{My()Theorem.tex, lines 141--206 and 876--906 --- correct
 stable weight algebra, conjectural pseudo-stable application.}  With the
 declared weights, the edge quotient is \(-t^{-2}\), the smoothing
 quotient is \(-d^2t^{-2}\), and their product is \(d^2t^{-4}\).
 The same computation on a pseudo-stable map space depends on \emph{(A1)}.

 \item \textbf{My()Theorem.tex, line 876 --- ambiguous.}  ``There are no
 automorphisms'' must mean that there is no moving infinitesimal
 automorphism.  The edge has a fixed infinitesimal scaling and the map has
 residual automorphism group \(\mu_d\); the latter gives \(1/d\).

 \item \textbf{My()Theorem.tex, lines 909--915 and 918--946 ---
 incomplete.}  Evenness of the surviving \(\psi\)-power follows from the
 dimension calculation above.  The last displayed equality is not a class
 identity, as the old draft itself observes; the needed integrated version
 is precisely \emph{(A3)}.

 \item \textbf{My()Theorem.tex, lines 951--986 --- incorrect/incomplete.}
 The edge-coloring calculation is explicitly marked wrong and cannot prove
 \emph{(A3)}.  It should be replaced by a genuine pullback,
 self-intersection, and excess-intersection computation for the gluing
 morphisms.

 \item \textbf{My()Theorem.tex, lines 997--1005 --- incomplete.}  The
 coefficient is correct for \(0\leq k\leq g-1\), conditional on
 \emph{(A3)}.  The printed upper limit \(g\) is wrong.  Formula
 \eqref{eq:correct-Ig-sum} is the corrected statement.

 \item \textbf{My()Theorem.tex, lines 1007--1024 and 1035--1096 ---
 incorrect as geometry, correct as later bookkeeping.}  Genus zero is an
 unstable vertex and genus one is a cusp vertex.  Neither is the integral
 \eqref{eq:correct-Ig-integral}.  The negative-factorial manipulations at
 lines 1043--1062 are invalid.  Formal coefficient extraction gives the
 same stated values rigorously.

 \item \textbf{My()Theorem.tex, lines 1098--1162 --- correct formal
 identity after correction of the derivation.}  The closed generating
 function is correct, but the proof must use generalized binomial series,
 not analytic Fubini or negative factorials.  See
 \eqref{eq:correct-I-generating}.

 \item \textbf{My()Theorem.tex, lines 1186--1201 --- conjectural.}  The
 convolution is algebraically correct once all vertex values and the
 \(1/d\) stack factor are accepted; its interpretation as a pseudo-stable
 Gromov--Witten invariant still requires \emph{(A1)--(A2)}.

 \item \textbf{My()Theorem.tex, lines 1225--1231 --- incorrect.}  The
 displayed differential equation for \(B\) fails already in degree zero.
 A correct elementary relation is
 \[
   \frac{B'(u)}{B(u)}=\frac1u-\frac12\cot(u/2).
 \]

 \item \textbf{FaberPandharipande.tex, lines 1301--1357 --- correct in the
 stable setting.}  These lines prove the one-edge reduction and the formula
 for \(C(g,d)\) only on the ordinary stable-map space.  Lines 1383--1412
 correctly give \(b_g\) and its generating series.  They do not justify a
 pseudo-stable virtual localization formula.

 \item \textbf{SchubertNewComp.tex, lines 622--675 --- correct but
 insufficient for maps.}  The construction concerns the moduli space of
 pseudo-stable curves.  It supplies neither a pseudo-stable map stack nor a
 perfect obstruction theory or virtual localization theorem.
\end{enumerate}
