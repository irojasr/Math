...
\begin{Ex}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.1\textwidth]{figs/fig1-1.png}%{figs/fig1-1-LineToTropLine.png}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.1\textwidth]{figs/fig1-1.png}%{figs/fig1-2-CubicToTropCubic.png}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.1\textwidth]{figs/fig1-1.png}%{figs/fig1-3-NodalCurveAndDualGraph.png}
...
\begin{Def}
    The \term{tropical semifield} is the set $\bT\defeq(\bR\cup\set{-\infty})$ equipped with tropical addition and multiplication:
    $$
    \begin{cases}
...
\end{tikzpicture}
\caption{Tropical Hypersurface from Valuation}
\label{fig:2-4-diagram-trop-hypersurf}
\end{figure}
...
\begin{Ex}
    ...
    \begin{figure}[h!]
        \centering
        \subcaptionbox{$X=V(x+y-1)$\label{fig:2.5-Variety}}{\includegraphics[width=0.25\textwidth]{figs/fig2-5-V(x+y-1).pdf}}\quad
        \subcaptionbox{$2$-amoeba of $X$\label{fig:2-6-2Amoeba}}{\includegraphics[width=0.25\textwidth]{figs/fig2-6-2amoeba.pdf}}\quad
        \subcaptionbox{Sequence of amoebas as $t\to\infty$\label{fig:2.7-ApproxAmoebas}}{\includegraphics[width=0.25\textwidth]{figs/fig2-7-ApproximatingAmoebas.pdf}}
        %\caption{This is the caption.}
        \label{fig:2.5-thru-2.7}
    \end{figure}
\end{Ex}
...
Consider a family of \red{of what, what is this family of?! Stuff? Curve in P1xP1 which eventually becomes P2?}\red{ask Renzo}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/fig1-1.png}%{figs/fig2-8-Degeneration.png}
    \caption{Degeneration of \red{stuff}}
...
%Last time, we discussed the classical (25 to 30 years old) ways to get to tropical geometry. 
...
\subsection{Tropical Arithmetics}%%%Based on 2-TropicalNumbers.pdf%%%
...
%Look at https://nonlinearalgebra.wordpress.com/2016/05/30/puiseux-series-example/
...
\begin{Ej}
    ...
    \begin{enumerate}[i)]
        ...
        \item[(2)] Assuming you know the tropical determinant of a matrix, devise a way to identify one job combination which reaches the optimum value. \aside{Actually! It is not necessary to know the value of the determinant.}
    \end{enumerate}
\end{Ej}
...
\subsubsection{Roots of Tropical Polynomials} %PAGE 4/11 TG4
...
%%% Page 7-11 TG4 not covered in class
...
\section{Day 6|20230901}%Vamos aquí 231229
...
%%%MAYBE DIVIDE INTO TWO EXAMPLES? 2401021039
\begin{Ex}
...
\subsection{Correspondence Theorems}%Begin TG5
Recall the maps
$$
\left\lbrace 
\begin{aligned}
...
\begin{Ex}
    Consider the polynomial
    $$q(X)=X^2+t^2X+1\in\bC\{\!\{t\}\!\}[X]\xrightarrow[]{\Trop}p(x)=x^2\oplus 2\odot x\oplus 0\in\bT[x].$$
...
\begin{Def}
    If $q(x)=\sum a_kx^k\in\bC[X]$ or $\bC\{\!\{t\}\!\}[x]$, then the \term{tropicalization} of $q$ is 
...
This is formal sum of monomials which in order to vanish needs at least two of the monomials to reach a minimum order of vanishing in order to cancel. This is equivalent to $\val(r)$ being a root of $\Trop(q)$. \par %%REVIEW
The other direction is substantially more difficult. This will be an instance of a realizability question. We have two cases, $r$ is a finite root or $r=\infty$. We will assume that $r$ is finite and do a proof by example. 
\end{ptcbp}
...
\begin{Ex}
    ...
    $$r_1=-1,\quad r_2=0,\word{and}r_3=1.$$
    We will now find a root $R_1\in\bC\{\!\{t\}\!\}$ of $q$ with $\val(R_1)=r_1$. For this to happen we requiere 
    $$R_1=yt^{-1}+Z\word{where}y\in\bC\word{and}Z\in\bC\{\!\{t\}\!\},\ \val Z>r_1.$$
    Plugging $R_1$ into $q$ leaves us with
    \begin{align*}
        q(R_1)&=t(yt^{-1}+Z)^3+(yt^{-1}+Z)^2+(yt^{-1}+Z)+t\\
        &=\un{y^3t^{-2}}+3y^2Zt^{-1}+3yZ^2+Z^3t+\un{y^2t^{-2}}+2yZt^{-1}+Z^2+yt^{-1}+Z+t
    \end{align*}
    where the underlined terms are the ones smaller than $t^{-1}$. Extracting their coefficients and equating to them zero gives us $y^3+y^2=0$ which means that $y=-1$. Plugging this back into our expression as $y$ we get 
    \begin{align*}
        &3Zt^{-1}-3Z^2+Z^3t-2Zt^{-1}+Z^2-t^{-1}+Z+t\\
        =&tZ^3-2Z^2+(t^{-1}+1)Z+(-t^{-1}+t).
    \end{align*}
    Tropicalizing this $R_1$ we get 
    $$\Trop R_1\mid_{y=-1}=1\odot z^{\odot 3}\oplus z^{\odot 2}\oplus (-1)\odot z\oplus (-1)=\min(3z+1,2z,z-1,-1).$$
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.5\textwidth]{figs/fig5-2EndOfProofFiniteCase.png}
        \caption{$\Trop R_1$ at $y=-1$.}
        \label{fig:5.2-EndOfProofFiniteCase}
    \end{figure}
    This has two roots $r_1=-1$ and $r_2=0$, one of which is larger than $-1$. So we now pick
    $$Z=y_1+Z_1\word{with}y_1\in\bC,\quad Z_1\in\bC\{\!\{t\}\!\}\word{and}\val(Z_1)\ge 0\footnote{It's actually an strict inequality but keyboard isn't working.}.$$
    We can plug this back into our expression to get 
    \begin{align*}
       &t(y_1+Z_1)^3-2(y_1+Z_1)^2+(t^{-1}+1)(y_1+Z_1)+(-t^{-1}+t)\\
       =&t\left((y_1+Z_1)^3+1\right)-2(y_1+Z_1)^2+(y_1+Z_1)+t^{-1}(y_1+Z_1+1)
    \end{align*}
    \red{ASK MAPLE CODE, how do we solve for $y_1$ here?}
\end{Ex} 

The question now is: how do we turn this idea into a formal proof?
\begin{enumerate}[i.]
    \item We work with one root at a time, starting with the rightmost one.
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.1\textwidth]{figs/fig1-1.png}%{figs/figInterim2-CancellationPhenomenon.png}
        \caption{Cancellation Phenomenon}
        \label{fig:Interim2-CancellationPhenomenom}
    \end{figure}    
...
\begin{enumerate}[i.]
    \item Let $p_k=(k,a_k)\in\bonj{0,d}\x\bT$.
...
\begin{Ex}
    ...
    $$p(x)=0\oplus1\odot x\oplus 1\odot x^{\odot 2}\oplus x^{\odot 3}\oplus 2\odot x^{\odot 4}\oplus 1\odot x^{\odot 5}.$$
    ...
    \end{figure}
    Looking at the diagram we see three segments:
    $$\obonj{0,1},\quad\obonj{1,4},\word{and}\obonj{4,5}$$
    which means that there are 3 roots. Their multiplicities will be $1,3$ and $1$ respectively as those are the lengths of the intervals.\par
    To actually find the root we look at the monomials and equate them. The monomial corresponding to the $0$ coefficient is just $0$ and the one with $1$ is $x+1$. From this we get the equation $0=x+1$. In a similar fashion we get the roots as follows:
    $$
    \left\lbrace
    \begin{aligned}
        &0=x+1\\
    ...
    \right.
    $$
    With this, we have that $-1,1$ are simple roots while $-1/3$ is a triple root.
\end{Ex}

\begin{Ex}
    Consider now the polynomial 
    $$p(x)=(-5)\oplus x\oplus  x^{\odot 2}\oplus(-4)\odot x^{\odot 4}\oplus(-10)\odot x^{\odot 6}.$$
    Ordinarily, we can find its roots to be $-5,0,2$ and $3$ with multiplicities $1,1,2$ and $2$ respectively.\par 
    Now let us place our $p_k$ points on top of the interval $\bonj{0,6}$ and see what happens.
    %FINISH THIS EXAMPLE 20240109
\end{Ex}
