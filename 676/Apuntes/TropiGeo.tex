...
\begin{ptcb}
\red{Since I don't understand exactly what to do, I'll do my best}\\
...
\end{itemize}
This is known as Maslov dequantization and with this we can see $(\bT,+,\.)$ as $(\set{-\infty}\cup\bR,\max,+)$. Also, we will abbreviate $\lim_{t\to\infty}T_t$ with $T_{t\to\infty}$
...
Concretely, elements here are Laurent series with coefficients having a common denominator. This field can be equipped with the valuation
$$\val: \bC\set{\set{t}}\to\set{-\infty}\cup\bR,\begin{cases}
    0\mapsto-\infty\\
...
\begin{enumerate}[i)]
    ...
    \item $\val(\al+\bt)\leq\min(\val(\al),\val(\bt))$.
\end{enumerate}
\end{Prop}
Equality holds when $\val(\al)\neq\val(\bt)$. So if we decide to define operations on $\set{-\infty}\cup\bR$ by inducing them from the operations on $\bC\set{\set{t}}$, then we obtain
\begin{align*}
...
$$x\diamondplus y=\begin{cases}
    \min(x,y)\word{when}x\neq y,\\
    [-\infty,\min(x,y)]\word{when}x=y.
\end{cases}$$
The only natural way to turn this into an operation is to define $x+y=\min(x,y)$. In conclusion, the field of Puiseux series with the order of vanishing and poles is congruent to $(\bT,+,\.)$ which in this case is $\left(\set{-\infty}\cup\bR,\min,+\right)$.
...
Observe that it is not possible to adjoin formal additive inverses. If $x+y=0_\bT$, then 
...
\begin{tikzpicture}
    ...
    $$(x+y)^n=x^n+y^n\word{and}n\max(x,y)=\max(nx,ny).$$
\end{Prop}
...
\begin{Ej}
    Do the following:
    \begin{itemize}
        \item[(5)] Provide an example of function with one simple zero at $-\infty$.
        \item[(5)] Now do one with a double zero at $-\infty$.
    \end{itemize}
\end{Ej}
\section{Day 3|20230825}

Recall that the last time we discussed the classical ways to get to tropical geoemtry (25 to 30 years old).
